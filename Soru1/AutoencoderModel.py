...
def TrainAutoEncoder(x_train, x_test, nb_epoch, comp_ratio, batch_size, c, snr, saver_step=50):
    ...
    print('\t|\t\t\t\t\t\t\t\t|')
    print('\t-----------------------------------------------------------------')
    tb = keras.callbacks.TensorBoard(log_dir='/content/Tensorboard/CompRatio{0}_SNR{1}'.format(str(comp_ratio), str(snr)))
    os.makedirs('/content/checkpoints/CompRatio{0}_SNR{1}'.format(str(comp_ratio), str(snr)), exist_ok=True)
    # os.makedirs('/content/checkpoints/CompRatio0.06_SNR10'.format(str(comp_ratio), str(snr), exist_ok=True))
    checkpoint = keras.callbacks.ModelCheckpoint(filepath='/content/checkpoints/CompRatio{0}_SNR{1}'.format(str(comp_ratio), str(snr))+'/Autoencoder.keras', monitor='val_loss', save_best_only=True)
